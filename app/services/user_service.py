...
def get_buyers_paginated(
    search: Optional[str],
    limit: int,
    offset: int,
) -> Tuple[list[Dict[str, Any]], int]:
    """
    Fetch buyers with pagination and optional search on name/phone.
    Returns (buyers_list, total_count).
    """
    conn = get_db_connection()
    cur = conn.cursor()
    try:
        ...
        print(f"Error fetching buyers (paginated): {e}")
        return [], 0
    finally:
        cur.close()
        conn.close()
